...
<body class="">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        <div class="flex items-center">
          <span class="font-bold text-xl">Edit-{{$ourpost->name}}</span>
        </div>
  ...
  <div class="bg-purple-50 flex items-center justify-center min-h-screen">
    ...
    <div>
      ...
      <input type="text" id="name" name="name" class="w-full px-4 py-2 rounded-lg border border-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500" value="{{$ourpost->name}}" />
      @error('name')
  ...
  <span class="block sm:inline">{{ $message }}</span>
    ...
    <div>
      ...
      <textarea id="description" name="description" rows="4" class="w-full px-4 py-2 rounded-lg border border-purple-300 focus:outline-none focus:ring-2 focus:ring-purple-500 " >{{$ourpost->description}}"</textarea>
      @error('description')
    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
  <span class="block sm:inline">{{ $message }}</span>
</div>
    ...
    <div>
      ...
      <input type="file" id="image" name="image" class="w-full text-purple-700" accept="image/*" value="{{$ourpost->image}}" />
      @error('image')
      <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
  <span class="block sm:inline">{{ $message }}</span>
</div>
